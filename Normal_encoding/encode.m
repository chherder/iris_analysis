...
function [template, mask, conf] = encode(polar_array,noise_array, nscales, minWaveLength, mult, sigmaOnf)
...
for k=1:nscales
    ...
    for i=0:(length2-1)
        ...
        %construct confidence information
        conf(h,ja+(2*k)-1) = real(E1(h, i+1));
        conf(h,ja+(2*k)) = imag(E1(h,i+1));
